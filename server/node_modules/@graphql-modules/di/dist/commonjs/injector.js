...
class Injector {
    constructor(options = {}) {
        ...
        this._injectorScope = this.options.injectorScope || types_1.ProviderScope.Application;
    ...
    provide(provider) {
        if (utils_1.isTypeProvider(provider)) {
            const options = Reflect.getMetadata(utils_1.PROVIDER_OPTIONS, provider);
    ...
    has(serviceIdentifier) {
        return (this.getScopeInstanceMap(types_1.ProviderScope.Application).has(serviceIdentifier) ||
            this.getScopeInstanceMap(types_1.ProviderScope.Session).has(serviceIdentifier) ||
    ...
    get(serviceIdentifier) {
        const applicationScopeInstanceMap = this.getScopeInstanceMap(types_1.ProviderScope.Application);
        const sessionScopeInstanceMap = this.getScopeInstanceMap(types_1.ProviderScope.Session);
        if (sessionScopeInstanceMap.has(serviceIdentifier)) {
            ...
            try {
                const dependencies = Reflect.getMetadata(utils_1.DESIGN_PARAMTYPES, RealClazz) || [];
                ...
                const propertyKeys = Reflect.getMetadata(utils_1.PROPERTY_KEYS, RealClazz) || [];
                for (const propertyKey of propertyKeys) {
                    const dependency = Reflect.getMetadata(utils_1.DESIGN_TYPE, RealClazz.prototype, propertyKey);
                    if (dependency) {
    ...
    getSessionInjector(session) {
        if (!this._sessionSessionInjectorMap.has(session)) {
            const sessionInjector = new Injector({
                ...
                injectorScope: types_1.ProviderScope.Session,
                hooks: this._hooks,
    ...
    call(fn, thisArg) {
        if ('hasMetadata' in Reflect && Reflect.hasMetadata(utils_1.DESIGN_PARAMTYPES, fn)) {
            const dependencies = Reflect.getMetadata(utils_1.DESIGN_PARAMTYPES, fn);
