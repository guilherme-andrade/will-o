...
const black = tinycolor('#0A1721');
const white = tinycolor('#FFFFFF');
const light = tinycolor('#F9F9F9');
...
class Theme {
  constructor() {
    ...
    this.white = white.toHexString();
    this.black = black.toHexString();
    ...
    this.sideModalHeaderBackground = this.white
    this.sideModalHeaderColor = tinycolor(this.sideModalHeaderBackground).isLight() ? this.black : this.white;
    ...
    this.mixins = {
      button: ({ variant, outline }) => `
        ...
        color: ${
          outline ? this[variant] : this.colorForBackground(this[variant])
        };
        background: ${outline ? "transparent" : this[variant]};
        font-weight: bold;
      ...
      input: () => `
        ...
        box-shadow: inset 0 0 10px ${this.backdropColor};
